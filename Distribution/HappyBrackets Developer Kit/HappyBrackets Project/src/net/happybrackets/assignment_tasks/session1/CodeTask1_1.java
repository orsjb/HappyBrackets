...
/**
 * This code is identical to the AboutTheCodeTasks example, which has the same functional behaviour as the
 * HelloWorldBeads example.
 *
 * Let's play with some of the code. Since this is our first time coding together, this first task is mainly dedicated
 * to seeing how well you understand Java code. We will only be looking at the task() function below. Ignore everything
 * else.
 *
 * Remember that to run this code you should control-click, or right-click on this file and select "Run
 * CodeTask1_1.main()". To re-run the code you can hit the green arrow up in the top right of the window in IntelliJ. If
 * the program is already running, then there should be a Run window below. You can press the 're-run' button, a green
 * circular arrow, to kill and restart the program.
 *
 * *** Step 1 ***
 *
 ...
 * Type this out. Don't copy and paste it. As you are typing the word Wave, notice that IntelliJ suggests code
 * completions. When IntelliJ suggests "WavePlayer", hit return instead of typing the rest of the word. This not only
 * completes the word but also imports the WavePlayer class into your project. Try to let IntelliJ do as much of the
 * coding for you as possible. Use it to autocomplete the constructor, new WavePlayer(etc.), and the Buffer.SINE
 * component.
 *
 ...
 * If you run the code after this change, nothing will be different. The "addInput()" command automatically connects a
 * mono signal (from the WavePlayer) to both sides of the stereo signal (into the Gain object).
 *
 ...
 * Run that.
 *
 * You should notice that you are changing which speaker your sound is coming out of (make sure you are listening in
 * stereo!). The first argument is the choice of input channel on the gain object. As you change this from 0 to 1 you
 * switch from the left speaker to the right speaker.
 *
 * *** Step 4 ***
 *
 * Now complete the task. Building on the code above, make changes to your final submission so that it has the following
 * properties:
